...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
	...
	} while (ret==UNZ_OK && ret!=UNZ_END_OF_LIST_OF_FILE && !self.cancelled);
    
    if(self.cancelled)
    {
        if ([self.delegate respondsToSelector:@selector(zipArchive:DidCancelFileAtPath:)])
        {
            [self.delegate zipArchive:self DidCancelFileAtPath:path];
        }
    }
