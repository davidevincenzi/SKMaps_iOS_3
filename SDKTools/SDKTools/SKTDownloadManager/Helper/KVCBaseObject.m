...
+ (id)objectForJSON:(NSString *)inputJSON {
    if (inputJSON) {
        NSDictionary *jDict = [NSJSONSerialization JSONObjectWithData:[inputJSON dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingMutableContainers error:nil];
...
+ (char *)typeOfPropertyNamed:(const char *)rawPropType {
    int k = 0;
    char *parsedPropertyType = (char *)malloc(sizeof(char *) * 16);
    if (*rawPropType == 'T') {
        rawPropType++;
    } else {
        rawPropType = NULL;
    ...
    if (*rawPropType == '@') {
        ...
        parsedPropertyType = strcpy(parsedPropertyType, "NSNumber\0");
        return parsedPropertyType;
        ...
        parsedPropertyType = strcpy(parsedPropertyType, "long long\0");
        return parsedPropertyType;
    }
    ...
    for (NSString *key in [inputDict allKeys]) {
        ...
        if (key) {
            ...
            if ([KVCBaseObject isPropertyTypeArray:propType]) {
                ...
                if ([childClass isSubclassOfClass:[KVCBaseObject class]] && [propertyValue respondsToSelector:@selector(allKeys)]) {
                    if ([[propertyValue allKeys] count]) { //prevent empty objects from beeing created
...
- (NSString *)objectToJson {
    NSError *error;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:[self objectToDictionary]
